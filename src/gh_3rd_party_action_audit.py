...
def search_github(org):
    ...
    regex_filter = fr"uses:\s*(?!\s*({org}|github|actions|\./\.github)/)"
    ...
    while True:
        ...
        for item in items:
            ...
            #     if re.search(regex_filter, fragment):
...
def write_markdown_summary(output_md, all_matches):
    with open(output_md, 'w') as f:
        ...
        # Query run 2025-03-17 12:45:43. Results for default branches only.

        f.write("Query run " + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ". Results for default branches only.\n\n")
        ...
        for line, count in sorted_lines:
            ...
            # Create a Confluence-compatible anchor link
        ...
        sorted_repos = sorted(repo_actions.items(), key=lambda x: len(x[1]), reverse=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GitHub Code Search with Advanced Filtering")

    parser.add_argument("org", help="GitHub organization/user name")
    parser.add_argument("--output-md", help="Output file for markdown summary", required=True)
